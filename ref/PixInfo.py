...
class PixInfo:
    ...
    def update_weights(self, relevant_imgs):
        ...
        relevant_matrix = []
        # e.g. relevant_imgs = [1, 2, 3] image 1 = index 0 in feature matrix
        for relevant_img in relevant_imgs:
            relevant_matrix.append(self.feature_matrix[relevant_img - 1])
        ...
        for i in range(89):  # go through each bin in column order
            ...
            column_average = sum / len(relevant_matrix)
            column_avgs.append(column_average)
        ...
        for i in range(89):
            std_sum = 0
            for j in range(len(relevant_matrix)):
                std_for_column = ((relevant_matrix[j][i] - column_avgs[i]) ** 2) / len(relevant_matrix)
                std_sum += std_for_column
            column_std = std_sum ** 0.5  # column_std = math.sqrt(std_sum)
            column_stds.append(column_std)
            # std = square root of ( ( (each column's cell number - column's average)^2 / total number of cells in column ) + do for the rest.. its summation )
        ...
        for i in range(89):
            if column_stds[i] == 0:
                if column_avgs[i] == 0:
                    ...
                    temp = column_stds.copy()
                    non_zero_column_stds = self.remove_zeros(temp)
                    column_stds[i] = (1 / 2) * (min(non_zero_column_stds))
